...
class CatalogController extends Controller
{
  ...
  public function themes(Request $request): JsonResponse
  {
    ...
    // Get root themes with children (only those with sets)
    if ($request->boolean('hierarchical', true)) {
      $themes = Theme::whereNull('parent_id')
        ->with(['children' => function ($query) {
          $query->withCount('sets')->having('sets_count', '>', 0);
    ...
    // Flat list (only themes with sets)
    $themes = $query->withCount('sets')->having('sets_count', '>', 0)->orderBy('name')->get();
  ...
  /**
   * Search autocomplete across all catalog categories.
   * Returns up to 5 results per category for quick navigation.
   */
  public function searchAutocomplete(Request $request): JsonResponse
  {
    $query = $request->get('q', '');
    $scope = $request->get('scope', 'all'); // all, sets, mocs, parts, minifigs, themes
    ...
    if (in_array($scope, ['all', 'minifigs'])) {
      $minifigs = Minifig::where(function ($q) use ($query) {
        $q->where('name', 'like', "%{$query}%")
