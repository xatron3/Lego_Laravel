...
class RebrickableController extends Controller
{
    ...
    /**
     * Reimport a single table (with forced updates of existing records).
     */
    public function reimport(string $table): JsonResponse
    {
        if (!isset($this->tableConfig[$table])) {
            return response()->json(['message' => 'Invalid table name.'], 400);
        }
        ...
        // Generate job ID
        $jobId = uniqid('reimport_', true);

        // Initialize progress in cache
        Cache::put("import_progress:{$jobId}", [
            'status' => 'pending',
            'progress' => 0,
            'message' => 'Reimport job queued (will update existing records)...',
            'table' => $table,
            'updated_at' => now()->toISOString(),
        ], now()->addHours(24));
        ...
        // Dispatch job with forceUpdate = true
        ImportRebrickableData::dispatch($table, $jobId, true);

        return response()->json([
            'message' => "Reimport job started for {$table} (will update existing records).",
            'job_id' => $jobId,
            'table' => $table,
        ]);
    }
    ...
    /**
     * Reimport all tables (with forced updates of existing records).
     */
    public function reimportAll(): JsonResponse
    {
        // Generate job ID
        $jobId = uniqid('reimport_all_', true);

        // Initialize progress in cache
        Cache::put("import_progress:{$jobId}", [
            'status' => 'pending',
            'progress' => 0,
            'message' => 'Reimport job queued for all tables (will update existing records)...',
            'table' => 'all',
            'updated_at' => now()->toISOString(),
        ], now()->addHours(24));
        ...
        // Dispatch job (null table means import all) with forceUpdate = true
        ImportRebrickableData::dispatch(null, $jobId, true);

        return response()->json([
            'message' => 'Reimport job started for all tables (will update existing records).',
            'job_id' => $jobId,
        ]);
    }
